...
2013) that included rapid diagnostic tests (RDTs).
...
identify surveys for the countries and years of interest.
...
recommended option is to download either the spss (.sav),
`fileFormat = "SV"`, or the flat file (.dat), `fileFormat = "FL"`
datasets. The flat is quicker, but there are still one or two very old
datasets that don’t read correctly, whereas the .sav files are slower to
read in but so far no datasets have been found that don’t read in
correctly. The household member recode (`PR`) reports the RDT status for
children under five.
...
function returns a vector with a file path to the saved location of the
downloaded datasets. These are read using
...
frame using the `labelled` class (see
[`haven::labelled`](https://haven.tidyverse.org/reference/labelled.html)
or our introduction vignette for more details). Variable labels are
stored in the `label` attribute.
...
This function works specifically with our lists of labelled
`data.frame`s. Labels are specified for each variable: for `hv024` all
labels are retained (concatenate) but for `hml35` labels across both
datasets to be “Neg” and “Pos”.
